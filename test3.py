...
while 1 + 1 == 2:
    ...
    random_num = random.uniform(60, 200)
    random_num = float(str(random_num)[:3])
    webhook_alarm = 'https://open.feishu.cn/open-apis/bot/v2/hook/a2fc9bdf-dc46-4b1a-9e0f-90a46e15f2d7'
    alarmdata = {
        "msg_type": "post",
        "content": {
            "post": {
                "zh-CN": {
                    "title": f"第{i}次刷新完成，下次刷新将在{random_num}后进行。",
                    "content": []
                }
            }
        }
    }
    response = requests.post(webhook_alarm, json=alarmdata)
    print(f"第{i}次刷新完成，下次刷新将在{random_num}后进行。")
    sleep(random_num)
    getlist = load()
    i = i + 1
    ...
    if len(difference) != 0:
        for item in difference:
            ...
            try:
                ...
                given_time = datetime.strptime(f"2024-{hooktime}", "%Y-%m-%d %H:%M:%S")
            current_time = datetime.now()
            ...
            if abs(time_difference) <= timedelta(minutes=30):
                hookdata = {
                    ...
                    "content": {
                        "post": {
                            "zh-CN": {
                                "title": f"{hooksource} / {hookheadline}",
                                "content": [
                                    [
                                        {
                                            "tag": "text",
                ...
                if any(word in hookdesc for word in content_list):
                    webhook_url = 'https://open.feishu.cn/open-apis/bot/v2/hook/d45bd493-e7dc-444c-9e03-43f1bdd6362c'
                else:
                    webhook_url = 'https://open.feishu.cn/open-apis/bot/v2/hook/c370f26a-50f1-4ba9-bd47-bdfb0e110972'
